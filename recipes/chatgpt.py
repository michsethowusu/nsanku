...
def translate_text_with_chatgpt(text, source_lang, target_lang, max_retries=5):
    ...
    for attempt in range(max_retries):
        try:
            completion = client.chat.completions.create(
                model="gpt-4.1",  # You can change to gpt-3.5-turbo, gpt-4, etc.
                messages=[
                    {
                        "role": "user",
                        "content": prompt
...
def translation_only(df, source_lang, target_lang):
    ...
    print(f"Translation: OpenAI ChatGPT API")
    ...
    for i, row in result_df.iterrows():
        ...
        translation = translate_text_with_chatgpt(text, source_lang, target_lang)
