...
# Initialize Anthropic client (using OpenAI-compatible endpoint if available)
# Note: You'll need to set CLAUDE_API_KEY in your .env file
client = OpenAI(
    base_url="https://api.anthropic.com/v1",  # Using Anthropic's official API
...
def translate_text_with_claude(text, source_lang, target_lang, max_retries=5):
    ...
    for attempt in range(max_retries):
        try:
            # Claude uses slightly different API parameters
            completion = client.chat.completions.create(
                model="claude-sonnet-4-5",  # You can change to claude-3-sonnet or claude-3-haiku
                messages=[
                    {
                        "role": "user",
                        "content": prompt
...
def translation_only(df, source_lang, target_lang):
    ...
    print(f"Translation: Claude API")
    ...
    for i, row in result_df.iterrows():
        ...
        translation = translate_text_with_claude(text, source_lang, target_lang)
